        ...
        return arrMemberFilter.map((item, index) => {
            ...
            if (!groupChatSelect.memberWaiting.includes(item._id))
                return (
                    ...
                    // />
                    <View className="flex flex-row mt-2 p-2 rounded-b-2xl rounded-t-2xl" key={item._id + '0' + index}>
                        <TouchableHighlight
                            key={item._id}
                            activeOpacity={0.6}
                ...
                return (
                    ...
                    <View className="flex flex-row mt-2 p-2 rounded-b-2xl rounded-t-2xl" key={item._id + index + '1'}>
                        <TouchableHighlight
                            activeOpacity={0.6}
                            underlayColor="#C6E4FF"
                            onPress={() => getAllChecked(item, index)}
